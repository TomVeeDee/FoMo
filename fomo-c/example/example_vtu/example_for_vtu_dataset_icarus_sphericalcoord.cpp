...
void processVtuFile(const std::string& inputFilename, const std::string& outputFilename) {
    try {
        ...
        if (!rhoArray || !v1Array || !v2Array || !v3Array || !pArray) {
            std::cerr << "Missing required data arrays in file: " << inputFilename << std::endl;
            return;
        }
        ...
        for (vtkIdType i = 0; i < numCells; ++i) {
            ...
            // Calculate the centroid of the cell manually
            double centroid[3] = {0.0, 0.0, 0.0};
            int numPointsInCell = cell->GetNumberOfPoints();
            
            // Sum the coordinates of each point in the cell
            ...
            double r = centroid[0], theta = centroid[1], phi = centroid[2];
            double x, y, z;
            sphericalToCartesian(r, theta, phi, x, y, z);

            // Scale coordinates (if necessary)
            std::vector<double> coordinates = {y * 696.0, z * 696.0, x * 696.0};

            // Get the cell data values for this cell
            float rho = rhoArray->GetValue(i);
            float pressure = pArray->GetValue(i);
            float T = (pressure / rho);  // Calculate temperature
