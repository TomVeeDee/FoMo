...
*/
const int y_pixel = 23;// 23 for AIA
...
//double lambda_width =1.0; // 0.14,0.3  // it is suggested to have a width of =-100 km/s in Doppler shift DY 29 Jan 2015
//const double speedoflight=GSL_CONST_MKSA_SPEED_OF_LIGHT; // speed of light
double lambda_width =1.0;//0.2; 
//double speedoflight=GSL_CONST_MKSA_SPEED_OF_LIGHT; 
...
//			reademissioncube(datacube, filename, &DT);
                        reademissioncube(datacube, filename, NULL);
			...
			goftcube=emissionfromdatacube(datacube);
             ...
             //            DT=triangulationfromdatacube(datacube);// only use the grid info
                      ...
                      //       while (DT.number_of_vertices() > 0) 
                       ...
                       //         }
                       //   Refresh DT_global is needed in case of Dynamics grid
                            triangulationfromdatacube_pt(datacube); // DT_global is changed DY 12 Feb 2015
...
//			writeemissioncube(goftcube,snapsave,&DT);
                        DT=DT_global;
                        writeemissioncube(goftcube,snapsave,&DT);                 
//                        writeemissioncube_pt(goftcube,snapsave);  
			if (commrank==0) cout << "Done!" << endl << flush;
