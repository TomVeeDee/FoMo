...
; time step by rotating around the axis of the cylinder for sausage mode, or by interpolating for kink mode
; INPUT:
...
if model eq 'ka2.24_hgres2d' then begin   ; Only for high-resolution model
   ...
   x_cube=fltarr(dimx, dimy, dimz)
   y_cube=fltarr(dimx, dimy, dimz)
   z_cube=fltarr(dimx, dimy, dimz)
...
; --------------------------------------------------------------------------------
   ...
   if mode eq 'sausage' then begin            ; Only for sausage mode
      ...
      for k=0,dimz-1 do dist_cube[*,*,k] = distance      
      for i=0,dimx-1 do x_cube[i,*,*] = gridx[i]
      for j=0,dimy-1 do y_cube[*,j,*] = gridy[j]
      for k=0,dimz-1 do z_cube[*,*,k] = gridz[k]
