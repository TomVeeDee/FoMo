...
pro readcubes,set=set,rh_cube_t,te_cube_t,vr_cube_t,vt_cube_t,vz_cube_t,br_cube_t,bz_cube_t,aa,gridx,gridy,gridz,gridz_ext,co,ce,ro,re,va,vae,bo,be,dimt,tarr,r0,ka_rt,wk_rt,rh_cube_t_ext,wave,dim00d,dim30d,dim45d,dim60d,mag=mag

; Reads cubes produced by datacubes_wt.pro

if keyword_set(set) eq 0 then begin
 print,'readcubes,set=set,rh_cube_t,te_cube_t,vr_cube_t,vt_cube_t,vz_cube_t,br_cube_t,bz_cube_t,aa,gridx,gridy,gridz,gridz_ext,co,ce,ro,re,va,vae,bo,be,dimt,tarr,r0,ka_rt,wk_rt,rh_cube_t_ext,wave,dim00d,dim30d,dim45d,dim60d [,mag=mag]'
   return
endif
...
;   dir = '/users/cpa/pantolin/Modeling/cubes/set2/'
;   dir2 = dir
   dir2 = '/volume1/scratch/set2/hgres/fe9/'
endif
...
if set eq 2 or set eq 23 or set eq 21 or set eq 3 or set eq 25 or set eq 26 or set eq 4 then siz = size(rh_cube)
...
if set ne 25 and set ne 26 then begin 
   ...
   if keyword_set(mag) then begin
      ...
      if set eq 4 then bz_cube_t = fltarr(siz[2],siz[3],dimt)
   ...
   if set eq 4 then vt_cube_t = fltarr(siz[2],siz[3],dimt)
...
if set eq 2 or set eq 23 or set eq 21 or set eq 3 or set eq 4 then begin
   for i=0,dimt-1 do begin
      ...
      if set eq 4 then vt_cube_t[*,*,i]=vt_cube[dimx/2,*,*]
      vz_cube_t[*,*,i]=vz_cube[dimx/2,*,*]
      if keyword_set(mag) then begin
         br_cube_t[*,*,i] = br_cube[dimx/2,*,*]
         if set eq 4 then bt_cube_t[*,*,i] = bt_cube[dimx/2,*,*]
