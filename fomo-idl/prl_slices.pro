...
pro prl_slices,set=set, mua_d=mua_d, ion=ion, w0=w0, imaging=imaging, dx=dx, dy=dy, dz=dz
...
; dz = spatial grid size along z: (gridz[dimz-1]-gridz[0])/dimz
  ...
  if set eq 2 or set eq 21 or set eq 22 or set eq 25 then dir = '/users/cpa/pantolin/Modeling/cubes/set2/'
  ...
  for i=0,nfiles-1 do begin
     ...
     if (file_test(checkfile) eq 0) then begin
        ...
        for j=0,n_elements(mua_d)-1 do begin
           ...
           if imaging eq 0 then begin
              integrateemission,emission=emission_goft_ext,n_gridx=n_gridy_ext,n_gridy=n_gridz_ext,ngrid=ngrid_ext,wave=wave,w0=w0,direction=direction,losvel=losvel_ext,imaging=imaging,image
           endif else begin
              integrateemission,emission=intens_ext,n_gridx=n_gridy_ext,n_gridy=n_gridz_ext,ngrid=ngrid_ext,wave=wave,w0=w0,direction=direction,losvel=losvel_ext,imaging=imaging,image
           endelse
