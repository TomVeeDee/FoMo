...
pro interpol_emiss_data,ne_s=ne_s,te=te,ion=ion,gotdir=gotdir,w0=w0,emission_goft=emission_goft,g_logte=g_logte,g_logne=g_logne,tstep=tstep,sav=sav,sdir=sdir,filenm=filenm,file_abund=file_abund,aia=aia,dkist=dkist,silent=silent

if keyword_set(ion) eq 0 then begin
   print,'interpol_emiss_data,ne_s=ne_s,te=te,ion=ion,gotdir=gotdir,w0=w0,emission_goft=emission_goft,g_logte=g_logte,g_logne=g_logne,tstep=tstep,sav=sav,sdir=sdir,filenm=filenm,file_abund=file_abund,aia=aia,dkist=dkist,silent=silent'
   return
endif
...
; line transition (or AIA or DKIST filter), and returns emissivity 
; function 
...
lookup_goft,ion=ion,w0=w0,gotdir=gotdir,n_e_lg=g_logne,logt=g_logte,goft_mat=goft_mat,watom= watom,nab=nab,filenm=filenm,aia=aia,dkist=dkist,silent=silent
