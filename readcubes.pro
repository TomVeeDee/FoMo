...
if set eq 2 or set eq 23 or set eq 21 or set eq 24 or set eq 3 then siz = size(rh_cube)

restore,dir+'params_'+ka_n+'.sav'

files = file_search(dir2+'rslt_slice_*.sav',count=nfiles,/fully_qualify_path)
restore,files[0]
dim00d = (size(image00d_ext))[2]
dim30d = (size(image30d_ext))[2]
dim45d = (size(image45d_ext))[2]
dim60d = (size(image60d_ext))[2]
...
if set eq 2 or set eq 21 or set eq 23 or set eq 22 or set eq 24 then begin
