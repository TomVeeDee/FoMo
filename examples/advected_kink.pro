...
; Define equilibrium (units: SI with length in Mm)
   ...
   m=1.
   harmonic = 1
   dimr = 85.    ; Take the final values (no real dimx, dimy, dimz anymore)
   dimth = 60.   
   dimz = 70.
   dimt = 16./harmonic
   rad = 1.
   ...
   ro = ne_in_cgs * 1.e6 * proton *norm^3/2.
   re = ne_out_cgs * 1.e6 * proton *norm^3/2.
  ...
  for j = 0, dimth-1 do begin
      ...
      ypos = rpos[i,j,k]*sin(thpos[i,j,k])     
        if rpos[i,j,k] lt rad then begin  ; specify loc, calculate eigenf, save position and eigenf, advect
          ...
          rhi_t = rhi_t/(1.e6 * proton *norm^3)*2
          ti_t = ti_t*proton/kboltz*norm^2*3./8.
          ...
          te_t = pe_t/rhe_t
          rhe_t = rhe_t/(1.e6 * proton *norm^3)*2
          te_t = te_t*proton/kboltz*norm^2*3./8.
