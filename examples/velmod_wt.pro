pro velmod_wt, waka_root=waka_root, ka_root=ka_root, gridx=gridx, gridr=gridr, dimt=dimt, reg3=reg3, vr_md=vr_md, kafix_0=kafix_0, aa, wk_rt, ka_rt, kafix, diml, dimz, theta, tarr, gridz, vr_t, vt_t, vz_t, rr_t, rtot_t, ptot_t, dispr, te_t, br_t, bt_t, bz_t, btot_t, mag=mag, nmode=nmode, no_standing=no_standing,uniform=uniform
...
; Calculates the advected modulation in time and space (r,z) of the MHD mode 
...
; of equations where the point came from. This root finding procedure
; should incorporate 3 equations, one for each dimension. Only the
; radial dimension has been considered here to estimate the how much
; time it takes. This procedure takes a LOT of time for the kink
; mode. 
...
if n eq 0 then kfixar = ([min(abs(ka_rt-kafix_0)),!c])[1] ; for ka closest to kafix_0
; kafix_0 = 2.244 or 1.255 (nmode = 0)
; kafix_0 = 0.03 (nmode = 1)
...
if keyword_set(uniform) then dimz = round(z_u*dimx/gridx[n_elements(gridx)-1]) else dimz = dimx
