...
PRO ch_synthetic, wmin, wmax, output=output, err_msg=err_msg, msg=msg, $
      ...
      goft=goft, ioneq_name=ioneq_name, dem_name=dem_name, $
      noprot=noprot, rphot=rphot, radtemp=radtemp, progress=progress, $
      no_sum_int=no_sum_int,  group=group
...
IF n_elements(logt_isothermal)  NE 0 THEN BEGIN
   ...
   ENDIF ELSE intunits = intunits+' cm-2 sr-1 s-1'
...
IF  keyword_set(goft) THEN intunits = intunits+' cm+3 sr-1 s-1' 
...
IF NOT keyword_set(noprot) THEN BEGIN
   pe_rat_all=proton_dens(ioneq_logt)
   noprot = 0 
   add_protons = 1
ENDIF ELSE add_protons = 0
...
IF n_elements(MASTERLIST) NE 0 THEN BEGIN 
   ...
   IF valid_num(MASTERLIST) THEN BEGIN 
      IF MASTERLIST EQ 1 THEN BEGIN 
         ...
         mname=pickfile(path=dir,filter='*',title='Select list of IONS File')
      ENDIF ELSE $
...
; lines in the range [wmin,wmax]

         read_wgfa2,wname,lvl1,lvl2,wvl1,gf1,a_value1,wgfaref

         IF keyword_set(all) THEN BEGIN
            anylines=WHERE((ABS(wvl1) GE wmin) AND (ABS(wvl1) LE wmax) AND $
                           (a_value1 NE 0.),nn)      
         ENDIF ELSE BEGIN
            anylines=WHERE((wvl1 GE wmin) AND (wvl1 LE wmax) AND $
                           (a_value1 NE 0.),nn)      
         ENDELSE
...
;---------------------------------------------------------------------------
      ...
      IF anylines[0] NE -1 THEN BEGIN ; do this ion
         ...
         IF KEYWORD_SET(verbose) THEN print,'Calculating  '+snote+' ('+trim(nn)+' lines)'
...
;
         ...
         ; added by kpd 10/10/03
          if dielectronic eq 1 then begin
             read_elvlc,dlvlcname,dl1a,dterm,dconf,dss,dll,djj,decm,deryd,decmth,derydth,deref
             mult(0)=2.*djj(0)+1.
             jj(0)=djj(0)
          endif
...
;read proton collisional data:
;------------------------------
            read_splups, pname, pstr, pref, /prot
         ENDELSE


; Read ionization and recombination files:
; -----------------------------------------

         read_ionrec,fname,rec_rate,ci_rate,temp_ionrec,luprec,lupci,status
         t_ioneq=ioneq_logt             ; Sets the temperature array for the ion abundances in the variable ionrec
...
IF photoexcitation THEN $
  input =CREATE_STRUCT(input, 'dilute', dilute, 'radtemp', radtemp)
         ...
         FOR i=0,nt-1 DO BEGIN
            ...
            IF NOT keyword_set(noprot) THEN BEGIN
               getmin=min(abs(ioneq_logt-alog10(temp[i])),i_t)
               pe_ratio=pe_rat_all[i_t]

inputs = create_struct(input, 'pe_ratio',pe_ratio) 
inputs = create_struct(inputs, 'prot_struc', pstr)

            ENDIF ELSE inputs = input

            pop_solver,inputs, temp[i],dens[i],pop

            pop = reform(pop)
            np = N_ELEMENTS(pop)

; dummy = where(pop EQ   0, ncc)
;IF ncc GT 0 THEN BEGIN 
;print, 'zero values for ion ' +snote
;END 

; dummy = where(pop LT  0, ncc)
;IF ncc GT 0 THEN BEGIN 
;print, 'negative values for ion ' +snote
;END 

;GDZ- avoid negative numbers 
            pops[i,0:np-1]=pop >  0.0
         ENDFOR
...
; ion if there are no lines that survive these tests.
              ...
              IF (n_elements(i_neg) lt n_elements(anylines)) or (n_elements(i_neg) eq n_elements(anylines) $
                               and i_neg(0) lt 0) THEN BEGIN     ; if there are lines left in the ion

                     IF i_neg(0) ge 0 THEN remove,i_neg,anylines

                     i_spl=where(lvl2[anylines] lt min(splstr.lvl2) or lvl2[anylines] gt max(splstr.lvl2))
...
;                            IF nt EQ n_elements(logt_isothermal) THEN BEGIN  ; Checks if ion T overlap with logt_isothermal
;                                list_int_new=fltarr(nt,nn_new) 
;                                FOR i=0,nt-1 DO BEGIN
;                                    list_int_new[i,0:nn_exist-1]=list_int[i,*]
;                                    list_int_new[i,nn_exist:nn_new-1]=t_list_int[i,*]
;                                ENDFOR 
;                                list_int=list_int_new
;                            ENDIF ELSE BEGIN                              ; If they do not overlap, stores only those who do
